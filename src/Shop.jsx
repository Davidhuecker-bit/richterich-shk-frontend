...
const money=cents=>(Number(cents||0)/100).toLocaleString("de-DE",{style:"currency",currency:"EUR"});
const priceLabel=t=>({sale_price:"Verkaufspreis",uvp:"UVP",reference:"Referenzpreis"}[t]||"Preis");
...
export default function Shop({products=[],api,onProjectCreated}){
  ...
  const visible=useMemo(()=>{
    ...
    return [...list].sort((a,b)=>{
      if(sort==="priceUp")return a.priceGrossCents-b.priceGrossCents;
      if(sort==="priceDown")return b.priceGrossCents-a.priceGrossCents;
  ...
  const total=cartLines.reduce((sum,x)=>sum+x.product.priceGrossCents*x.line.quantity,0);
  ...
  return <section className="shop-shell">
    ...
    {products.length===0?<div className="shop-empty">
      ...
      <div className="shop-grid">{visible.map(p=><article key={p.id} className="shop-card">
        ...
        <div className="shop-card-copy"><small>{p.category} · {p.brand}</small><h3>{p.title}</h3><p className="sku">{p.manufacturerNumber||p.sku}</p>
          <div className="price-line"><div><span>{priceLabel(p.priceType)}</span><strong>{money(p.priceGrossCents)}</strong></div><span className={p.priceStatus==="confirmed"?"price-ok":"price-warn"}>{p.priceStatus==="confirmed"?"aktuell geprüft":"erneut prüfen"}</span></div>
          ...
          <p className="source-note">{p.sourceLabel||p.supplier}{p.priceCheckedAt?" · Stand "+new Date(p.priceCheckedAt).toLocaleDateString("de-DE"):""}</p>
        </div>
      </article>)}</div>
    </>}
    ...
    {detail&&<div className="modal-backdrop" onClick={()=>setDetail(null)}><div className="product-modal" onClick={e=>e.stopPropagation()}>
      ...
      <div className="product-modal-copy"><small>{detail.category} · {detail.brand}</small><h2>{detail.title}</h2><p>{detail.description}</p>
        <div className="detail-price"><span>{priceLabel(detail.priceType)}</span><strong>{money(detail.priceGrossCents)}</strong></div>
        ...
        <button className="primary-cta" onClick={()=>{add(detail);setDetail(null)}}>In den Warenkorb →</button>
        <p className="source-note">Datenquelle: {detail.sourceLabel||detail.supplier}. Preisstand: {detail.priceCheckedAt?new Date(detail.priceCheckedAt).toLocaleString("de-DE"):"nicht angegeben"}.</p>
      </div>
    </div></div>}
    ...
    {checkout&&<div className="modal-backdrop" onClick={()=>setCheckout(false)}><div className="cart-drawer" onClick={e=>e.stopPropagation()}>
      ...
      <div className="cart-lines">{cartLines.length?cartLines.map(({product,line})=><div className="cart-line" key={product.id}>
        ...
        <div><strong>{product.title}</strong><span>{product.brand} · {money(product.priceGrossCents)}</span></div>
      ...
      {cartLines.length>0&&<><div className="cart-total"><span>Zwischensumme</span><strong>{money(total)}</strong></div><p className="cart-hint">Endpreis, Versand und Lieferfähigkeit werden vor dem verbindlichen Auftrag nochmals bestätigt.</p>
